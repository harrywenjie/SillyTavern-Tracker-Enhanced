...
const RESERVOIR_ALLOWED_LOCATIONS = new Set(["cervix", "vaginal"]);
const LOCATION_PRIORITY = Object.freeze({
	vaginal: 1,
	cervix: 2,
});
...
function reconcileReservoirs(options) {
	...
	for (const entry of existingState) {
		...
		const normalizedLocation = typeof entry.location === "string" ? entry.location.toLowerCase() : "";
		const reservoirLocation = normalizedLocation || "vaginal";
		entry.location = reservoirLocation;
		if (!RESERVOIR_ALLOWED_LOCATIONS.has(reservoirLocation)) {
			diagnostics.actions.push({ type: "reservoir_pruned_location", location: reservoirLocation });
			continue;
		}
	...
	if (ejaculationEvent) {
		const normalizedLocation = typeof ejaculationEvent.location === "string" ? ejaculationEvent.location.toLowerCase() : "";
		const reservoirLocation = normalizedLocation || "vaginal";
		if (!RESERVOIR_ALLOWED_LOCATIONS.has(reservoirLocation)) {
			diagnostics.actions.push({ type: "reservoir_skipped_location", location: reservoirLocation });
		} else {
			ejaculationEvent.location = reservoirLocation;
			...
			if (mergeTarget) {
				...
				mergeTarget.location = resolveReservoirLocation(mergeTarget.location, reservoir.location);
	...
	return { reservoirs: updatedState };
}
...
function createReservoirEntry(ejaculationEvent, anchor, engineState) {
	...
	const location = (ejaculationEvent.location || "vaginal").toLowerCase();
	const locationBonus = LOCATION_BONUS[location] || 0;
...
function resolveReservoirLocation(existing, incoming) {
	const existingKey = typeof existing === "string" ? existing.toLowerCase() : "";
	const incomingKey = typeof incoming === "string" ? incoming.toLowerCase() : "";
	if (!incomingKey) {
		return existingKey;
	}
	if (!existingKey) {
	...
	const existingPriority = LOCATION_PRIORITY[existingKey] ?? 0;
	const incomingPriority = LOCATION_PRIORITY[incomingKey] ?? 0;
	return incomingPriority >= existingPriority ? incomingKey : existingKey;
}
...
function evaluateConception(options) {
	...
	const viableReservoirs = reservoirs.filter((entry) => {
		const location = typeof entry.location === "string" ? entry.location.toLowerCase() : "";
		if (!location) {
			return true;
		}
		return RESERVOIR_ALLOWED_LOCATIONS.has(location);
	});
	if (viableReservoirs.length === 0) {
		return { conceived: false };
	}
	...
	const candidates = viableReservoirs
		.map((entry) => ({
			partner: entry.partner,
			potency: (entry.volumeScore / 100) * (entry.motilityScore / 100),
			locationBonus: LOCATION_BONUS[(typeof entry.location === "string" ? entry.location.toLowerCase() : "")] || 0,
		}))
