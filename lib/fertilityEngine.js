...
const INTERNAL_KEYS = Object.freeze({
	...
	LAST_CREAMPIE: "lastCreampie",
	LAST_CREAMPIE_OCCURRED: "lastCreampieOccurred",
	LAST_CREAMPIE_PARTNER: "lastCreampiePartner",
	LAST_CREAMPIE_LOCATION: "lastCreampieLocation",
	LAST_CREAMPIE_CONTRACEPTION: "lastCreampieContraception",
	LAST_CREAMPIE_NOTES: "lastCreampieNotes",
});
...
function buildSchemaIndex(schema) {
	...
	const index = {
		...
		lastCreampieFieldId: pick(INTERNAL_KEYS.LAST_CREAMPIE),
		lastCreampieOccurredFieldId: pick(INTERNAL_KEYS.LAST_CREAMPIE_OCCURRED),
		lastCreampiePartnerFieldId: pick(INTERNAL_KEYS.LAST_CREAMPIE_PARTNER),
		lastCreampieLocationFieldId: pick(INTERNAL_KEYS.LAST_CREAMPIE_LOCATION),
		lastCreampieContraceptionFieldId: pick(INTERNAL_KEYS.LAST_CREAMPIE_CONTRACEPTION),
		lastCreampieNotesFieldId: pick(INTERNAL_KEYS.LAST_CREAMPIE_NOTES),
	};
...
function processCharacter(context) {
	...
	if (!isFemale) {
		...
		characterNode[index.lastCreampieFieldId] = null;
	...
	const lastCreampie = normalizeLastCreampie(characterNode[index.lastCreampieFieldId], index);
...
function normalizeLastCreampie(raw, index) {
	if (!raw || typeof raw !== "object") {
		return null;
	}
	const occurredRaw = getNode(raw, index.lastCreampieOccurredFieldId);
	const occurred = typeof occurredRaw === "string" ? occurredRaw.trim().toLowerCase() === "true" : Boolean(occurredRaw);
	if (!occurred) {
		return null;
	}
	const partner = extractString(getNode(raw, index.lastCreampiePartnerFieldId)) || "unknown";
	const location = (extractString(getNode(raw, index.lastCreampieLocationFieldId)) || "vaginal").toLowerCase();
	let contraception = getNode(raw, index.lastCreampieContraceptionFieldId);
	...
	const notes = extractString(getNode(raw, index.lastCreampieNotesFieldId)) || "";
...
function reconcileReservoirs(options) {
	...
	if (lastCreampie) {
		const reservoir = createReservoirEntry(lastCreampie, anchor, engineState);
...
function createReservoirEntry(lastCreampie, anchor, engineState) {
	const partner = lastCreampie.partner || "unknown";
	...
	const modifiers = interpretContraception(lastCreampie.contraception);
	const locationBonus = LOCATION_BONUS[lastCreampie.location] || 0;
...
function evaluateConception(options) {
	...
	const twinChance = determineTwinChance(candidates, eggPotency, lastCreampie);
...
function determineTwinChance(partners, eggPotency, lastCreampie) {
	let chance = 0.03;
	...
	if (lastCreampie?.notes && /ivf|fertility|treatment/i.test(lastCreampie.notes)) {
		chance += 0.05;
	}
